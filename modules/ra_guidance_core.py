...
class RaGuidanceCore:
    ...
    def __init__(self, guardian=None, event_bus=None):
        ...
        self.logger.info("🧭 RaGuidanceCore активирован: автопилот + реакция на мир")
        ...
        # ---------------------------
        # Подписка на EventBus (без дубликатов)
        # ---------------------------
        if self.event_bus:
    ...
    def analyze_energy(self, text):
        positive = set(["любовь", "свет", "надежда", "радость", "дух", "энергия"])
        negative = set(["боль", "страх", "злость", "пустота", "одиночество"])
    ...
    def create_intent(self, text):
        ...
        # Сохраняем в память
        asyncio.create_task(self.memory.append("system", f"Guidance: {decision}", layer="long_term"))
    ...
    def guidance(self, text):
        ...
        if self.guardian and hasattr(self.guardian, "approve_guidance"):
            ...
            if not approved:
                self.logger.warning("🛡 Guardian отклонил итоговое решение")
                result["action"] = "пауза_для_безопасности"

        # 🔥 После каждого guidance даём энергию Thinker'у и основной поток
