...
# Подключение модулей света (они должны лежать рядом с этим файлом)
import сердце
import вселенная
import время

# Путь к папке data независимо от того, где запущен скрипт
...
def случайная_мудрость():
    ...
    # Подключаем модуль сердца для резонанса
...
# Боевые напоминания (оставляем, когда пойдёшь в реальный режим)
...
# Инициализация сердечного потока
мое_сердце = сердце.Сердце(имя="Ты")
...
while True:
    ...
    # Просто выводим напоминание от времени
    print(время.Время().ожидать("здесь_и_сейчас"))
    time.sleep(5)  # проверяем задачи чаще, чтобы не пропустить
