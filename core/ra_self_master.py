...
_police = None
try:
    from modules import ra_police as police_module  # pkg import if package
...
class RaSelfMaster:
    ...
    def __init__(self, manifest_path="data/ra_manifest.json"):
        ...
        # police client is lazy-loaded later
        self.police = None
    ...
    def sync_manifest(self):
        """
        Проверяет и обновляет активные модули и дату последнего обновления.
        Если манифест отсутствует — создаёт.
        """
        if not self.manifest:
            self.manifest = {"active_modules": []}
        # Обновляем список активных модулей из автолоадера (если есть)
        if self.autoloader:
            loaded = list(self.autoloader.modules.keys())
            if loaded:
                # объединяем и сохраняем уникально
    ...
    # --- Команды для полиции (при наличии) ---
