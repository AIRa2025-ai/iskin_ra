...
class RaCoreMirolub:
    ...
    def __init__(self):
        ...
        # --- Подписка на энергию для всех модулей ---
        self.energy.subscribe(self._on_energy_update)

        # Запускаем поток энергии
        self.energy.start()

    def _on_energy_update(self, уровень: int):
        """Callback для всех аспектов Ра при обновлении энергии"""
        try:
            # мозг реагирует на поток энергии
            if hasattr(self.искр, "сознание") and self.искр.сознание:
                self.искр.сознание.update_energy(уровень)

            # световые потоки
            if hasattr(self.искр, "ядро") and self.искр.ядро:
                self.искр.ядро.adjust_energy(уровень)

            if hasattr(self.искр, "поток") and self.искр.поток:
                self.искр.поток.adjust_energy(уровень)

            # сердце и эмоции
            if hasattr(self.искр, "сердце") and self.искр.сердце:
                self.искр.сердце.react_energy(уровень)

            # дух и раскрытие
            if hasattr(self.искр, "дух") and self.искр.дух:
                self.искр.дух.influence_energy(уровень)

            # память / хроники
            if hasattr(self.искр, "память") and self.искр.память:
                self.искр.память.log_energy(уровень)

        except Exception as e:
            logging.warning(f"Ошибка при обработке энергии в callback: {e}")
