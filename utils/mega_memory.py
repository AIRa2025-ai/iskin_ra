...
def restore_from_mega():
    ...
    try:
        ...
        if not archive_id:
            log("⚠️ Архив памяти не найден")
            notify("⚠️ Архив памяти не найден")
            return
        ...
        log(f"❌ Ошибка восстановления памяти: {e}")
        notify(f"❌ Ошибка восстановления памяти: {e}")
